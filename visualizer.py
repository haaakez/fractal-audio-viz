...
Audio separation is automatic when Demucs is installed.  If it is not, the
script still accepts one song and derives vocal/instrument energy controls
from frequency bands, so pre-rendered vocal and instrumental files are not
required.
...
KEYFRAME_CACHE_SCHEMA = "scaled-double-native-crop-colour-v4"
...
def _demucs_stems(audio_path: Path, output_dir: Path, mode: str) -> Optional[tuple[Path, Path]]:
    ...
    if not available:
        ...
        print("Demucs is not installed; using the single-file spectral fallback.")
        return None
    ...
    if result.returncode != 0:
        ...
        print("Demucs failed; using the single-file spectral fallback.")
        if result.stderr:
    ...
    if stems is None:
        ...
        print("Demucs produced no usable stems; using the single-file spectral fallback.")
    return stems
...
def _resample_features(values: Any, frame_count: int, fps: int, sample_rate: int) -> Any:
    ...
    source_times = np.arange(values.size, dtype=np.float64) * (sample_rate / fps) / sample_rate
...
def _component_has_signal(component: Any, full_mix: Any) -> bool:
    ...
    full_mix = np.nan_to_num(np.asarray(full_mix, dtype=np.float64))
    component_p95 = float(np.percentile(np.abs(component), 95.0))
    mix_p95 = float(np.percentile(np.abs(full_mix), 95.0))
    if component_p95 <= 1e-7:
        return False
    # Demucs stems and spectral proxies have different absolute scales, so
    # use this only as a conservative guard against an effectively silent stem.
    if mix_p95 > 1e-7 and component_p95 < mix_p95 * 0.015:
        return False
...
def _spectral_fallback(samples: Any, sample_rate: int, fps: int) -> tuple[Any, Any]:
    ...
    frame_count = math.ceil(len(samples) * fps / sample_rate)
    return (
        _resample_features(vocal_energy, frame_count, fps, sample_rate),
        _resample_features(instrumental_energy, frame_count, fps, sample_rate),
    )
...
def analyse_audio(
    ...
    if separated_controls:
        ...
        print("Audio controls: vocals drive gradient; instruments drive zoom.")
    else:
...
def render_video(
    ...
    render_width = max(16, int(round(width * render_scale * fractal_scale)))
    render_height = max(16, int(round(height * render_scale * fractal_scale)))
    command = [
        "ffmpeg",
        "-y",
        "-loglevel",
    ...
    cache_identity = _field_renderer_cache_identity(renderer)
    total_frames = features.frame_count
    # Coverage is independent from source resolution.  With a small
    # fractal_scale this may upscale between keyframes, which is intentional:
    # it gives a predictable fast 4K encode instead of multiplying keyframes.
    max_zoom_factor = max(1.05, keyframe_factor)
    max_log_factor = math.log10(max_zoom_factor)
    print(
        f"Native keyframe source: {render_width}x{render_height}; "
    ...
    if renderer != "python":
        ...
        if native_library is None:
            ...
            try:
                # Common-scale double perturbation handles the normal video
                # range through 1e120.  Tune the reusable FloatExp BLA table
                # to that hand-off depth instead of the initial e0 viewport;
                # otherwise its composition bounds are needlessly pessimistic
                # for e120--e9800 frames.
                bla_start_log = min(float(np.max(zooms)), 120.0)
    ...
    try:
        ...
        while frame_index < total_frames:
            ...
            while (
                ...
                and float(zooms[chunk_end]) - chunk_log_zoom < max_log_factor
            ):
                chunk_end += 1
            ...
            if cache_dir is not None:
                ...
                try:
                    ...
                    if cached.shape == (render_height, render_width):
                        field = np.asarray(cached, dtype=np.float32)
            ...
            if field is None:
                ...
                if cache_path is not None:
                    np.save(cache_path, field, allow_pickle=False)
            ...
            for frame_index in range(chunk_start, chunk_end):
                ...
                if native_library is not None and resample == "bilinear":
                    rgb = _crop_and_colourise_native(
                        field,
                        width,
                        height,
                        relative_zoom,
                    ...
                    view = _crop_and_resize(field, width, height, relative_zoom, resample)
                    if native_library is not None:
                        rgb = _colourise_native(
                            view,
                            current_iter,
                            phase,
                            vocal,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--separation",
        choices=("auto", "demucs", "none"),
        default="auto",
        help="use Demucs when available; auto falls back to one-file spectral analysis",
    )
    parser.add_argument(
        "--render-scale",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="additional native fractal resolution multiplier; 0.25 prioritises speed over source detail",
    )
    parser.add_argument(
        "--keyframe-factor",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="native expansion compatibility setting for deep renders (1-32)",
    )
    parser.add_argument(
        "--series-block",
        type=int,
        default=32,
        help="compatibility setting retained for older native callers",
    )
    parser.add_argument(
        "--renderer",
...
def main() -> None:
    ...
    features = analyse_audio(args.audio, args.sample_rate, args.fps, args.separation)
    zooms = _zoom_plan(
    ...
    if args.estimate:
        render_width = max(16, int(round(args.width * args.render_scale * args.fractal_scale)))
        render_height = max(16, int(round(args.height * args.render_scale * args.fractal_scale)))
        print(
            f"Estimate: {render_width}x{render_height} fractal source, "
            f"{_keyframe_count(zooms, args.keyframe_factor)} keyframes, "
            f"{args.video_preset} encoder."
        )
        return
    render_video(
        args.audio,
